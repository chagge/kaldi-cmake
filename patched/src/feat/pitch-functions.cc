...
void ExtractFrame(const VectorBase<double> &wave,
  ...
  int32 frame_shift = opts.NccfWindowShift();
  int32 frame_length = opts.NccfWindowSize();
  int32 outer_max_lag = iround<kaldi::BaseFloat>(opts.resample_freq / opts.min_f0) +
      iround<kaldi::BaseFloat>(opts.lowpass_filter_width/2);
...
void SelectLag(const PitchExtractionOptions &opts,
  ...
  int32 lag_size = 1 + iround<kaldi::BaseFloat>(log((max_lag + delta_lag) / (min_lag - delta_lag))
...
void Compute(const PitchExtractionOptions &opts,
  ...
  double outer_max_lag = 1.0 / (1.0 * opts.min_f0) +
      (opts.upsample_filter_width/(2.0 * opts.resample_freq));
  int32 num_max_lag = iround<kaldi::BaseFloat>(outer_max_lag * opts.resample_freq) + 1;
  int32 num_lags = iround<kaldi::BaseFloat>(opts.resample_freq * outer_max_lag) -
      iround<kaldi::BaseFloat>(opts.resample_freq *  outer_min_lag) + 1;
  int32 start = iround<kaldi::BaseFloat>(opts.resample_freq  * outer_min_lag),
      end = iround<kaldi::BaseFloat>(opts.resample_freq / opts.min_f0) +
      iround<kaldi::BaseFloat>(opts.lowpass_filter_width / 2);
