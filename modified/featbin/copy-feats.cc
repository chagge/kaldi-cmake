...
bool LoadMatrix(const string& _path, Matrix<BaseFloat>& _matrix, size_t _vect_dim, size_t& _vect_num) {
	const size_t file_size = boost::filesystem::file_size(_path);
	_vect_num = file_size / (_vect_dim * sizeof(float));
	if (file_size == (_vect_num * _vect_dim * sizeof(float))) {
		_matrix.Resize(_vect_num, _vect_dim, kaldi::MatrixResizeType::kSetZero);
		std::ifstream file(_path, std::ios_base::binary);
		if (file.good()) {
			file.read((char*) _matrix.Data(), file_size);
			return true;
		} else {
			return false;
		}
...
int main(int argc, char *argv[]) {
  try {
	...
	const char* dir_dst = "g:\\temp\\mfcc_dbg";
