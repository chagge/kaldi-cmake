...
void WalkAlign(const TransitionModel &trans_model, const std::vector<int32> &alignment) {
	for (size_t i = 0; i < alignment.size(); i++) {
		int32 trans_id = alignment[i];
		int32 state_id = trans_model.TransitionIdToTransitionState(trans_id);
		int32 phone_id = trans_model.TransitionStateToPhone(state_id);
		printf("ph: %3d  st: %3d  tr: %3d", phone_id, state_id, trans_id);
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    for (; !reader.Done(); reader.Next()) {
      ...
      ////////////////////////// DEBUG //////////////////////////////////
      //
      WalkAlign(trans_model, alignment);
      //
      ///////////////////////////////////////////////////////////////////
